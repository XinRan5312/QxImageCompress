 ...
 */
public class QxCompressActivity extends AppCompatActivity implements QxPhoneImgCompress.CompressListener {
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == RESULT_OK) {
            if (requestCode == CAMERA_REQESTCODE) {
                QxPhoneImgCompress imgCompress = QxPhoneImgCompress.getInstance(this);
                imgCompress.setCompressListener(this);
                imgCompress.starCompress(Uri.fromFile(imageFile).toString(), 600, 800, 100);
            }
        }
    }
    ...
    @Override
    public void onCompressEnd(QxPhoneImgCompress.CompressResult compressResult) {
        Log.d(TAG, "onCompressEnd outPath:" + compressResult.getOutPath());
        if (compressResult.getStatus() == QxPhoneImgCompress.CompressResult.RESULT_ERROR)//压缩失败
            return;
