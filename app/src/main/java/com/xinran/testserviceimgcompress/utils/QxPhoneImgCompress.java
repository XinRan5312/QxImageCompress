 ...
 */
public class QxPhoneImgCompress {

    private static QxPhoneImgCompress instance = null;
    ...
    private QxPhoneImgCompress(Context context) {
        this.context = context;
    }

    public static QxPhoneImgCompress getInstance(Context context) {
        if (instance == null) {
            synchronized (QxPhoneImgCompress.class) {
                if (instance == null)
                    instance = new QxPhoneImgCompress(context.getApplicationContext());
            }
        }
        return instance;
    }
    ...
    public String compressImage(String srcImageUri, int outWidth, int outHeight, int maxFileSize) {
        ...
        if (srcWidth > maxWidth || srcHeight > maxHeight) {
            ...
            //同理如果输入比率大于输出比率,则最终输出的高度以maxWidtht为准()
